...
def get_gallery_images(brand: str = "ei8ht_plants", page_size: int = 100) -> list[str]:
    ...
    API エラー時は stale キャッシュ → [] の順でフォールバックする。
    """
    ...
    try:
        ...
        logger.warning("Drive API error in get_gallery_images(%s): %s", brand, e)
...
def get_home_gallery_images() -> list[str]:
    ...
    try:
        ...
        logger.warning("Drive API error in get_home_gallery_images: %s", e)
        return cache.get_stale(cache_key) or []
