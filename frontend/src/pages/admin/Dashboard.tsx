    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 16, marginBottom: 40 }}>
        {statCards.map(({ label, value, to, urgent }) => (
          <Link key={to} to={to} style={{ textDecoration: 'none', display: 'block', padding: '20px 24px', background: urgent ? '#fff8f0' : '#ffffff', border: `1px solid ${urgent ? '#e8c870' : '#dddde8'}`, borderRadius: 12 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 16 }}>
        {ITEMS.map(({ to, label, desc }) => (
          <Link key={to} to={to} style={{ display: 'block', padding: 24, background: '#ffffff', border: '1px solid #dddde8', borderRadius: 14, textDecoration: 'none', color: '#1c2417' }}>
