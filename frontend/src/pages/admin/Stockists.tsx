    ...
    <div>
      ...
      {editing && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 500, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <form onSubmit={saveEdit} style={{ background: '#fffcf6', borderRadius: 14, padding: 32, width: 480, maxWidth: '90vw', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            ...
            <div style={{ gridColumn: '1 / -1', display: 'flex', gap: 10, justifyContent: 'flex-end', marginTop: 4 }}>
              <button type="button" onClick={() => setEditing(null)} style={{ padding: '10px 20px', background: 'none', border: '1px solid #ddd4c0', borderRadius: 8, cursor: 'pointer', fontSize: 14 }}>キャンセル</button>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {stockists.map((s) => (
          <div key={s.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 18px', background: '#fffcf6', border: '1px solid #ddd4c0', borderRadius: 10, gap: 12 }}>
            <div style={{ flex: 1 }}>
            ...
            <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
              <button onClick={() => startEdit(s)} style={{ padding: '8px 16px', border: '1px solid #ddd4c0', borderRadius: 8, fontSize: 13, background: 'none', cursor: 'pointer', color: '#3a4535' }}>編集</button>
